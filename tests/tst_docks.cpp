...
/// These macros are workaround for clang-format-18 messing calls with KDDW_CO_AWAIT
#define EVENT_LOOP(millis) \
    KDDW_CO_AWAIT          \
    Platform::instance()->tests_wait(millis)

#define WAIT_FOR_RESIZE(window) \
    KDDW_CO_AWAIT               \
    Platform::instance()->tests_waitForResize(window)
...
#define WAIT_FOR_EVENT(window, event) \
    KDDW_CO_AWAIT                     \
    Platform::instance()->tests_waitForEvent(window, event)
