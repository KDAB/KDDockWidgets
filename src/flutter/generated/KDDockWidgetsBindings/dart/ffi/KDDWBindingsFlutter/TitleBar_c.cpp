...
void TitleBar_wrapper::free_impl()
{
...
void TitleBar_wrapper::free_impl_nocallback()
{
    ::KDDockWidgets::flutter::TitleBar::free_impl();
}
...
bool TitleBar_wrapper::isExpicitlyHidden() const
{
    if (m_isExpicitlyHiddenCallback) {
        const void *thisPtr = this;
        return m_isExpicitlyHiddenCallback(const_cast<void *>(thisPtr));
    } else {
        return ::KDDockWidgets::flutter::TitleBar::isExpicitlyHidden();
    }
}
bool TitleBar_wrapper::isExpicitlyHidden_nocallback() const
{
    return ::KDDockWidgets::flutter::TitleBar::isExpicitlyHidden();
}
...
{
    [&] {auto targetPtr = fromPtr(thisObj);auto wrapperPtr = dynamic_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::TitleBar_wrapper*>(targetPtr);if (wrapperPtr) {    return wrapperPtr->free_impl_nocallback();} else {    return targetPtr->free_impl();} }();
}
// geometry() const
...
{
    return [&] {auto targetPtr = fromPtr(thisObj);auto wrapperPtr = dynamic_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::TitleBar_wrapper*>(targetPtr);if (wrapperPtr) {    return wrapperPtr->isExpicitlyHidden_nocallback();} else {    return targetPtr->isExpicitlyHidden();} }();
}
...
void c_KDDockWidgets__flutter__TitleBar__registerVirtualMethodCallback(void *ptr, void *callback, int methodId)
{
    ...
    case 930:
        ...
        break;
    case 932:
        wrapper->m_free_implCallback = reinterpret_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::TitleBar_wrapper::Callback_free_impl>(callback);
        break;
    case 934:
