...
bool DockWidget_wrapper::isExpicitlyHidden() const
{
    if (m_isExpicitlyHiddenCallback) {
        const void *thisPtr = this;
        return m_isExpicitlyHiddenCallback(const_cast<void *>(thisPtr));
    } else {
        return ::KDDockWidgets::flutter::DockWidget::isExpicitlyHidden();
    }
}
bool DockWidget_wrapper::isExpicitlyHidden_nocallback() const
{
    return ::KDDockWidgets::flutter::DockWidget::isExpicitlyHidden();
}
...
{
    return [&] {auto targetPtr = fromPtr(thisObj);auto wrapperPtr = dynamic_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::DockWidget_wrapper*>(targetPtr);if (wrapperPtr) {    return wrapperPtr->isExpicitlyHidden_nocallback();} else {    return targetPtr->isExpicitlyHidden();} }();
}
// isMaximized() const
...
void c_KDDockWidgets__flutter__DockWidget__registerVirtualMethodCallback(void *ptr, void *callback, int methodId)
{
    ...
    case 957:
        ...
        break;
    case 958:
        wrapper->m_isExpicitlyHiddenCallback = reinterpret_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::DockWidget_wrapper::Callback_isExpicitlyHidden>(callback);
        break;
    case 959:
