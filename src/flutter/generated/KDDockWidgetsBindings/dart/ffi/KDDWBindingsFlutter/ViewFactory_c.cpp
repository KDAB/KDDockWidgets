...
QString ViewFactory_wrapper::classicIndicatorsPath() const
{
    if (m_classicIndicatorsPathCallback) {
        const void *thisPtr = this;
        return *m_classicIndicatorsPathCallback(const_cast<void *>(thisPtr));
    } else {
        return ::KDDockWidgets::flutter::ViewFactory::classicIndicatorsPath();
    }
}
QString ViewFactory_wrapper::classicIndicatorsPath_nocallback() const
{
    return ::KDDockWidgets::flutter::ViewFactory::classicIndicatorsPath();
}
...
{
    const auto &result = new Dartagnan::ValueWrapper<QString> { [&] {auto targetPtr = fromPtr(thisObj);auto wrapperPtr = dynamic_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::ViewFactory_wrapper*>(targetPtr);if (wrapperPtr) {    return wrapperPtr->classicIndicatorsPath_nocallback();} else {    return targetPtr->classicIndicatorsPath();} }() };
    return result;
}
...
void c_KDDockWidgets__flutter__ViewFactory__registerVirtualMethodCallback(void *ptr, void *callback, int methodId)
{
    ...
    switch (methodId) {
    case 237:
        wrapper->m_classicIndicatorsPathCallback = reinterpret_cast<KDDockWidgetsBindings_wrappersNS::KDDWBindingsFlutter::ViewFactory_wrapper::Callback_classicIndicatorsPath>(callback);
        break;
    case 238:
