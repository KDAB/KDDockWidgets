...
public:
    ...
    enum Flag {
        ...
        Flag_LazyResize = 64, /// The dock widgets are resized in a lazy manner. The actual resize only happens when you release the mouse button.
        Flag_TabsHaveCloseButton = 128, /// Tabs will have a close button. Equivalent to QTabWidget::setTabsClosable(true).
        Flag_DoubleClickMaximizes = 256, /// Double clicking the titlebar will maximize a floating window instead of re-docking it
        Flag_TitleBarHasMaximizeButton = 512, /// The title bar will have a maximize/restore button when floating. This is mutually-exclusive with the floating button (since many apps behave that way).
