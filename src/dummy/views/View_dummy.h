...
class DOCKS_EXPORT View_dummy : public View
{
public:
    using View::close;
    using View::height;
...
private:
    Q_DISABLE_COPY(View_dummy)
};

inline View_dummy *asView_dummy(View *view)
{
    if (!view)
        return nullptr;
    return static_cast<View_dummy *>(view);
}

inline View_dummy *asView_dummy(Controller *controller)
{
    if (!controller)
        return nullptr;

    return static_cast<View_dummy *>(controller->view());
}
