    ...
    , d(new Private(name, options, layoutSaverOptions, this))
{
    ...
    if (name.isEmpty())
        spdlog::error("Name can't be null");
...
{
    if (other == this) {
        spdlog::error("Refusing to add dock widget into itself {}", ( void * )other);
        return;
    }

    if (!other) {
        spdlog::error("dock widget is null");
        return;
    }

    if (!DockRegistry::self()->affinitiesMatch(other->affinities(), d->affinities)) {
        spdlog::error("Refusing to dock widget with incompatible affinity. {} {}", other->affinities(), affinities());
        return;
    }

    if ((other->options() & DockWidgetOption_NotDockable)
        || (options() & DockWidgetOption_NotDockable)) {
        spdlog::error("Refusing to dock non-dockable widget {}", ( void * )other);
        return;
    }
    ...
    if (group) {
        if (group->containsDockWidget(other)) {
            spdlog::error("Already contains {}", ( void * )other);
            return;
        }
    } else {
        if (view()->isRootView()) {
                                                 ...
                                                 InitialOption initialOption)
{
    ...
    if (auto fw = floatingWindow()) {
        ...
        spdlog::error("Couldn't find floating nested window");
    }
}
...
bool DockWidget::setFloating(bool floats)
{
    ...
    if (floats) {
        ...
        if (isTabbed()) {
            auto group = d->group();
            if (!group) {
                spdlog::error("DockWidget::setFloating: Tabbed but no group exists", ( void * )this);
                Q_ASSERT(false);
                return false;
            }
...
bool DockWidget::isTabbed() const
{
    if (Core::Group *group = d->group()) {
        ...
        if (!isFloating())
            spdlog::error("DockWidget::isTabbed() Couldn't find any tab widget.");
        return false;
    }
}
...
void DockWidget::onResize(QSize)
{
    if (isOverlayed()) {
        if (auto group = d->group()) {
            ...
            spdlog::error("Overlayed dock widget without group shouldn't happen");
        }
    }
}
...
Core::DockWidget *DockWidget::deserialize(const LayoutSaver::DockWidget::Ptr &saved)
{
    ...
    if (dw) {
        ...
        if (dw->affinities() != saved->affinities) {
            spdlog::error("Affinity name changed from {} to {}", dw->affinities(), "; to", saved->affinities);
...
{
    if (floatingWindow()) {
        spdlog::error("Call this function only before having a floating window");
    } else {
        d->m_flags = flags;
    }
}
